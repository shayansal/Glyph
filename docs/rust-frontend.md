...
- `SemanticSsrServer` provides the first server-side path for accessibility HTML, capability-over-HTTP responses, secure session-derived capability requests, and streaming world update events.
- `AxumSsrAdapter` exposes a real Axum/Tokio SSR router for world JSON, accessibility HTML, capability invocation, secure capability headers, and server-sent world update streams. `serve_localhost()` starts an ephemeral local server for tests and development smoke checks.
...
This is still a kernel, not a polished app framework. It now has macro, fine-grained reactive, host, policy studio, conformance, interop, accessibility-frame, watched hot-reload contracts, live Axum/Tokio SSR with session-derived capability policy, production renderer command frames, GPU pipeline plans, primitive pipeline routing, headless pixel output, a real native `wgpu::Surface` presenter with product-loop routing, richer text shaping metadata, screenshot conformance, devtools replay, AI personalization previews, runtime state bridges, a long-running dev manager model, a polling watcher/supervisor/executor model, and generated mobile project templates. The next layer should add live OS watcher subscriptions, deeper hardware GPU primitive execution, full text/IME integration, typed production SSR/RPC transports, Rust mobile FFI packaging, and a polished visual devtools application.
