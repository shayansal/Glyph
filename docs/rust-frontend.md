...
- `SemanticSsrServer` provides the first server-side path for accessibility HTML, capability-over-HTTP responses, secure session-derived capability requests, typed capability RPC envelopes, and streaming world update events.
- `AxumSsrAdapter` exposes a real Axum/Tokio SSR router for world JSON, accessibility HTML, capability invocation, secure capability headers, and server-sent world update streams. `serve_localhost()` starts an ephemeral local server for tests and development smoke checks. Typed SSR RPC is available in-process through `SsrTypedRpcEnvelope` and `handle_typed_capability_rpc`.
...
- `glyphspace-dev` backs `gx dev` with a long-running process-manager model for native/web/SSR targets, watcher state, browser/window launch state, diagnostics, devtools heartbeats, and state preservation. It now includes project config parsing, supervisor health reports, polling fingerprint file watching, native notification backend contracts, command execution, safe SSR restart with preserved state snapshots, an integrated watcher/restart runtime loop, watcher subscriptions, owned child-process launch/poll/termination reports, incremental reload plans, and crash recovery diagnostics. `--report` and `--once` keep CI/test invocations finite; normal `gx dev` remains alive.
- Product component contracts cover props, slots/children, typed events, lifecycle hooks, accessible form/table/list/menu primitives, runtime transactions, undo/redo, patch persistence, sync conflicts, Rust-owned no-JS web runtime, native/mobile integration plans, visual devtools, diagnostic bundles, and distribution readiness.
...
This is still a kernel, not a polished app framework. It now has macro, fine-grained reactive, host, policy studio, conformance, interop, accessibility-frame, watched hot-reload contracts, live Axum/Tokio SSR with session-derived capability policy and typed RPC envelopes, production renderer command frames, GPU pipeline plans, primitive shader compilation plans, headless pixel output, a real native `wgpu::Surface` presenter with product-loop routing, richer text shaping metadata, screenshot conformance, devtools replay, AI personalization previews, runtime state bridges, a long-running dev manager model, a polling watcher/supervisor/executor/runtime-loop model, and generated mobile project templates. The next layer should bind live OS watcher subscriptions into the CLI loop, execute per-primitive GPU pipelines, add full text/IME integration, typed network RPC transports, Rust mobile FFI packaging, and a polished visual devtools application.
