...
Glyphspace is between Phase 0 and Phase 1. The reference kernel exists and is tested across core schema/model, policy-safe patches, layout, accessibility, Rust app runtime, SSR contracts, renderer command frames, headless pixel output, and conformance reports. The project is not yet a product-ready native/web/mobile framework because the hardware hosts, polished devtools, package/release story, and mobile projects are still incomplete.
...
- Phase 2: Product hosts. Build desktop `winit`/`wgpu`, browser WebGPU, no-JS Rust/WASM bootloader, and polished `gx dev`.
