...
- `glyphspace-render-wgpu` with a native swapchain presentation contract plus an actual `winit` + `wgpu::Surface` presenter that configures swapchain usage, render passes, resources, presentation, and screenshot readback bindings. Product-loop frame routing, GPU glyph upload plans, hardware command-frame byte encoding, text atlas payloads, draw-pass partitioning, real `wgpu::Buffer`/texture upload binding, shader input layouts, indexed draw plans, primitive pipeline routing for cards/panels, dots/glows, edges, text, and focus/policy overlays, command-frame raster snapshots, and browser WebGPU parity reports are now executable.
...
- Axum/Tokio SSR adapter for world JSON, accessibility HTML, capability POST, streamed world updates, and secure capability requests that derive policy context from auth/session headers or explicit `SsrAuthSession` values.
...
| Renderer | Command-frame/headless pixel plus real `winit`/`wgpu::Surface` binding, product-loop routing, encoded GPU frame bytes, real buffer/texture uploads, shader input draw plans, and primitive pipeline routing | Replace the shared placeholder WGSL with mature per-primitive card/dot/edge/text/focus shaders |
| Web/WASM | Rust/WASM kernel plus thin JS glue | No-JS app authoring, Rust bootloader, WebGPU host parity |
| SSR | Axum adapter, tested routes, and session-derived capability policy contracts | Deployment templates, typed RPC transport, auth provider adapters, and secure audit storage |
