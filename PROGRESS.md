...
- Latest full validation: `bash scripts/check.sh` passed after Production tranche 6.
...
- Productization tranche added native product-loop frame routing to `WinitWgpuSurfacePresenter`, GPU glyph upload plans, deterministic command-frame raster snapshots for cards/dots/edges/text/focus rings, browser WebGPU parity reports, richer text shaping metadata for fallback/emoji/RTL/ligatures/wrapping, real `gx dev` command execution and safe SSR restart state preservation, native notification backend contracts, product component props/slots/events/lifecycle, accessible form/table/list/menu primitives, runtime transactions with undo/redo and patch persistence, sync conflict reports, formal policy language parsing with enterprise layers and last-known-safe fallback, native accessibility bridge descriptors and screen-reader snapshots, Rust-owned no-JS web runtime contracts, native/mobile integration plans, visual devtools product frames, diagnostic bundles, and distribution readiness tracking.
- Production tranche 2 added `HardwareGlyphPipeline` for deterministic command-frame encoding into vertex/index/instance/uniform/text-atlas byte payloads, hardware draw-pass partitioning for cards/dots/edges/text/focus-policy overlays, deterministic pixel snapshot contracts, `DevOrchestrator` process bootstrap reports, `LiveWatcherStream` native-notification reload batches, compiler diagnostic parsing, `InvalidFixtureCorpus` coverage for invalid worlds/patches/policies/layouts, and `ApiStabilityReport` for public Rust surface, feature flags, extension roots, semver guarantees, and formal error-code coverage.
- Production tranche 3 added `WgpuSurfaceBindingPlan` and `WinitWgpuSurfacePresenter::bind_hardware_pipeline`, which allocate/write real vertex/index/instance/uniform `wgpu::Buffer` resources and a text-atlas `wgpu::Texture` from encoded command frames. It also added `NativeOsWatcherBridge` for OS create/modify/remove/rename events, native watcher capability reports in `gx dev --report`, and kernel fixture/API stability sections in `gx conformance --out`.
- Production tranche 4 added `HardwareShaderInputPlan`, vertex/instance shader layouts, indexed draw ranges, and a surface WGSL path that consumes uploaded glyph vertex and instance buffers instead of relying only on placeholder vertex indices. It also added `LongRunningDevSupervisor` for crash/heartbeat/restart events with preserved semantic state, plus `gx conformance --artifact-dir` for versioned kernel, renderer, policy, and manifest report files.
- Production tranche 5 added `PrimitivePipelineSet` with explicit card/panel, dot/glow, edge, text-atlas, and focus/policy overlay pipeline descriptors plus draw routing from hardware indexed draws. It also added `DevRuntimeLoop`, which integrates native OS watcher events, reload batches, long-running restart reports, diagnostics, devtools heartbeats, and state preservation, and added secure SSR capability sessions through `SsrAuthSession`/`SsrCapabilityRequest` with CSRF, tenant, permission, and audit metadata.
- Production tranche 6 added `PrimitiveShaderRegistry` and `PrimitivePipelineCompilationPlan` with specialized WGSL modules and bind-group/depth/blend/sample contracts for cards/panels, dots/glows, edges, text-atlas rendering, and focus/policy overlays. It also added `DevRuntimeLoop::watcher_subscriptions` plus owned child-process launch/poll/termination reports, and added typed SSR capability RPC through `SsrTypedRpcEnvelope`, `register_typed_rpc_capability`, `handle_typed_capability_rpc`, typed output decoding, semantic patches, and audit records.
...
- Bind `PrimitivePipelineCompilationPlan` to actual per-primitive `wgpu::RenderPipeline` instances inside `WinitWgpuSurfacePresenter`, connect the runtime child-process/watch subscription contracts to the `gx dev` CLI loop, add Rust FFI packaging for the generated native mobile projects, and build a polished visual devtools UI backed by the Devtools Studio frame model.
