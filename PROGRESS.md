...
- Mature the hardware shader passes into per-primitive card/dot/edge/text/focus pipelines, connect `NativeOsWatcherBridge` to the long-running platform watcher event source, add generated native mobile projects, authenticated SSR capability sessions, and a polished visual devtools UI backed by the new Devtools Studio frame model.
