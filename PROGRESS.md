...
- Bind encoded glyph GPU buffers to real hardware shader draws on `wgpu::Surface`, connect `LiveWatcherStream` to the native OS watcher backend, add generated native mobile projects, authenticated SSR capability sessions, and a polished visual devtools UI backed by the new Devtools Studio frame model.
